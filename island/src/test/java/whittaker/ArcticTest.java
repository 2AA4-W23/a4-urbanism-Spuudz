...
public class ArcticTest {
    ...
    public void genWhittaker(){
        ...
        island=arctic.genWhittaker(island);
        assertEquals("boreal",(island.getTiles(0).getProperties().get("tile_type")));
        assertEquals("alpine",island.getTiles(1).getProperties().get("tile_type"));
        assertEquals("alpine",(island.getTiles(2).getProperties().get("tile_type")));
